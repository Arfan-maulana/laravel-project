    ...
    <x-app-layout>
        ...
        <div class="overflow-hidden bg-white shadow-sm dark:bg-gray-800 sm:rounded-lg">
            <div class="px-6 pt-6 mb:w-1/2 2xl:w-1/3">
                @if (request('search'))
                <h2 class="pb-3 text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">Search results for : {{request('search')}}</h2>
                @endif
                    ...
                    <tbody>
                        ...
                        <tr class="odd:bg-white odd:dark:bg-gray-800 even:bg-gray-50 even:dark:bg-gray-700">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <p>{{$user->todos->count()}}
                                    <span>
                                        <span class="text-green-600 dark:text-green-400">{{$user->todos->where('is_complete', true)->count()}}</span>/
                                        <span class="text-green-600 dark:text-green-400">{{$user->todos->where('is_complete', false)->count()}}</span>
                                    </span>
                                </p>
                            </td>
                            <td class="px-6 py-4">
