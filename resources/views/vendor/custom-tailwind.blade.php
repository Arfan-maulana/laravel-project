                    ...
                    <span class="relative z-0 inline-flex rounded-md shadow-sm">
                        ...
                        @foreach ($elements as $element)
                            ...
                            @if (is_string($element))
                                <span aria-disabled="true">
                                    <span
                                        class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium leading-5 text-gray-700 cursor-default dark:text-gray-200 bg-gray-50 dark:bg-gray-700">{{ $element }}</span>
                                </span>
                            @endif
                                ...
                                @foreach ($element as $page => $url)
                                    @if ($page == $paginator->currentPage())
                                        <span aria-current="page">
                                            <span
                                                class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium leading-5 text-gray-900 cursor-default dark:text-gray-200 bg-indigo-50 dark:bg-indigo-900">{{ $page }}</span>
                                        </span>
                                    @else
                                        <a href="{{ $url }}"
                                            class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium leading-5 text-gray-700 transition duration-150 ease-in-out dark:text-gray-200 bg-gray-50 dark:bg-gray-700 hover:text-gray-500 focus:z-10 active:bg-gray-100 active:text-gray-700"
                        ...
                        @if ($paginator->hasMorePages())
                            <a href="{{ $paginator->nextPageUrl() }}" rel="next"
                                class="relative inline-flex items-center px-2 py-2 -ml-px text-sm font-medium leading-5 text-gray-500 transition duration-150 ease-in-out dark:text-gray-200 bg-indigo-50 dark:bg-indigo-900 rounded-r-md hover:text-gray-400 focus:z-10 active:bg-gray-100 active:text-gray-500"
                        ...
                        @else
                            <span aria-disabled="true" aria-label="{{ __('pagination.next') }}">
                                <span
                                    class="relative inline-flex items-center px-2 py-2 -ml-px text-sm font-medium leading-5 text-gray-500 cursor-default bg-gray-50 dark:bg-gray-700 rounded-r-md"
                                    aria-hidden="true">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd"
                                            d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z"
